...
module "step_function" {
  ...
  definition = templatefile("${path.module}/sfn_definition.json.tmpl",
    { datasyncS3ToEFS = var.datasync_task_s3_efs,
      BatchTaskArn    = var.batch_task_arn,
      JobQueue        = local.job_queue.arn,
  datasyncEFSToS3 = var.datasync_task_efs_s3 })
